...
import { FargoLogo } from "@/components/fargo/FargoLogo";
import { FARGO_NAV_GROUPS } from "@/components/fargo/nav";
import { cn, initials } from "@/lib/utils";

interface FargoSidebarProps {
  onNavigate?: () => void;
  userName?: string;
  userEmail?: string;
}

export function FargoSidebar({ onNavigate, userName, userEmail }: FargoSidebarProps) {
  ...
  return (
    <div className="flex h-full w-64 flex-col bg-white dark:bg-slate-950">
      ...
      <nav className="flex-1 space-y-6 overflow-y-auto px-3 py-5" aria-label="Fargo navigation">
        {FARGO_NAV_GROUPS.map((group) => (
          <div key={group.title}>
                  ...
                  <li key={item.href}>
                    <Link
                      ...
                      className={cn(
                        ...
                        "focus:outline-none focus-visible:ring-2 focus-visible:ring-fargo-500",
                        active
                          ? "bg-fargo-700 text-white dark:bg-fargo-700"
      ...
      <div className="space-y-3 border-t border-slate-200 p-3 dark:border-slate-800">
        {userName && (
          <div className="flex items-center gap-3 rounded-xl px-2 py-1.5">
            <div className="flex size-9 shrink-0 items-center justify-center rounded-full bg-fargo-700 text-xs font-bold text-white">
