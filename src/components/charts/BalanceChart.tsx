...
export function BalanceChart() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch(`/api/balance-history?range=${range}`, { cache: "no-store" });
        ...
        <div className="flex items-center gap-1 rounded-lg bg-slate-100 p-1 dark:bg-slate-800" role="tablist" aria-label="Balance history range">
          {RANGES.map((r) => (
            <button
              ...
              className={cn(
                "rounded-md px-2.5 py-1 text-xs font-semibold transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500",
                range === r.value
            ...
            <AreaChart data={points} margin={{ top: 8, right: 12, bottom: 0, left: 0 }}>
              <defs>
                <linearGradient id="balanceFill" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#4471b3" stopOpacity={0.35} />
                  <stop offset="95%" stopColor="#4471b3" stopOpacity={0.02} />
                </linearGradient>
              </defs>
              ...
              />
              <Area type="monotone" dataKey="balance" stroke="#4471b3" strokeWidth={2.5} fill="url(#balanceFill)" />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </div>
