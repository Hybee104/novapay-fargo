...
export async function GET() {
  ...
  const rows = await prisma.supportConversation.findMany({
    where: { userId: user.id, bank: BANK_FARGO },
...
export async function POST(req: Request) {
  ...
  const conversation = await prisma.supportConversation.create({
    data: {
      ...
      bank: BANK_FARGO,
      subject,
      agentName: FARGO_AGENT_PROFILE.displayName,
      messages: {
        create: [
          { senderRole: "user", senderName: `${user.firstName} ${user.lastName}`, body: message },
          { senderRole: "agent", senderName: FARGO_AGENT_PROFILE.displayName, body: agentReply },
        ],
      },
    },
    include: {
